...
@libentry()
@triton.autotune(
    configs=runtime.get_tuned_config("groupnorm"),
    key=["group_size", "HW"],
)
...
def group_norm_kernel(
    ...
    # pass 1: accumulate mean/var terms in chunks of BLOCK_HW_SIZE
    sum_x = 0.0
    sum_x2 = 0.0
    ...
    var = sum_x2 / num_elements - mean * mean
    rstd = 1.0 / tl.sqrt(var + eps)

    # preload weight/bias once
    if W is None:
        weight = 1
    else:
    ...
    # pass 2: normalize/store in chunks
    for off in range(0, HW, BLOCK_HW_SIZE):
        hw_offset = off + tl.arange(0, BLOCK_HW_SIZE)
        hw_mask = hw_offset < HW
        xy_offset = pid * num_elements + group_offset[:, None] * HW + hw_offset[None, :]
        xy_mask = wb_mask[:, None] & hw_mask[None, :]
        X_val = tl.load(X + xy_offset, mask=xy_mask, other=0.0).to(tl.float32)
        x_hat = tl.where(xy_mask, (X_val - mean) * rstd, 0.0)
