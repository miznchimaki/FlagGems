...
@triton.jit
def conv2d_forward_kernel_gather(
    input_pointer,
    weight_pointer,
    output_pointer,
    bias_pointer,
    ...
    n_mask = (in_n_point_value < in_n)[:, None]
    co_mask = (output_c_offset < out_per_group_c)[None, :]
    ...
    for h in range(weight_height):
        for w in range(weight_width):
            ...
            inp_ptrs = (
                input_pointer
                + (input_c_stride * offs_ci)[None, :]
                + (input_height_stride * input_height_offset)[:, None]
                + (input_width_stride * input_width_offset)[:, None]
            )
            wgt_ptrs = (
                weight_pointer
                + (weight_c_stride * offs_ci)[:, None]
                + weight_height_stride * h
                + weight_width_stride * w
            )

            for ci in range(BLOCK_CI_COUNT):
                c_mask = offs_ci + ci * BLOCK_CI < weight_c
                input_mask = spatial_mask & c_mask[None, :]
                weight_mask = c_mask[:, None] & co_mask

                input_block = tl.load(inp_ptrs, mask=input_mask)
                weight_block = tl.load(wgt_ptrs, mask=weight_mask)

                accum += tl.dot(input_block, weight_block, allow_tf32=False)

                inp_ptrs += BLOCK_CI * input_c_stride
                wgt_ptrs += BLOCK_CI * weight_c_stride
...
def conv2d_forward_kernel(
    ...
    dilation_width: tl.constexpr,
    groups: tl.constexpr,
    BLOCK_NI_HO_WO: tl.constexpr,  # acts as BLOCK_WO
    BLOCK_CI: tl.constexpr,
    BLOCK_CO: tl.constexpr,
):
    # Grid: (cdiv(out_width, BLOCK_WO), cdiv(out_per_group_c, BLOCK_CO), groups * in_n * out_height)
    # N and H_out are decomposed into grid dim 2, so input access is regular strided (no gather).
    pid_wo = tl.program_id(0)
    pid_co = tl.program_id(1)
    pid_n_ho_g = tl.program_id(2)

    out_per_group_c = out_c // groups
    pid_group = pid_n_ho_g % groups
    pid_n_ho = pid_n_ho_g // groups
    n = pid_n_ho // out_height
    ho = pid_n_ho % out_height

    wo_offs = pid_wo * BLOCK_NI_HO_WO + tl.arange(0, BLOCK_NI_HO_WO)
    co_offs = pid_co * BLOCK_CO + tl.arange(0, BLOCK_CO)
    ci_offs = tl.arange(0, BLOCK_CI)

    # Base pointers (all scalars)
    input_base = (
        input_pointer + n * input_n_stride + pid_group * weight_c * input_c_stride
    )
    weight_base = weight_pointer + pid_group * out_per_group_c * weight_n_stride

    accum = tl.zeros((BLOCK_NI_HO_WO, BLOCK_CO), dtype=tl.float32)
    BLOCK_CI_COUNT = (weight_c + BLOCK_CI - 1) // BLOCK_CI

    wo_mask = (wo_offs < out_width)[:, None]
    co_mask = (co_offs < out_per_group_c)[None, :]

    for h in range(weight_height):
        hi = ho * stride_height + h * dilation_height - padding_height
        h_valid = (hi >= 0) & (hi < input_height)
        # Clamp hi to valid range for pointer safety (scalar, preserves stride pattern).
        # Invalid reads are excluded by h_valid in spatial_mask.
        hi_safe = tl.minimum(tl.maximum(hi, 0), input_height - 1)

        for w in range(weight_width):
            wi_offs = wo_offs * stride_width + w * dilation_width - padding_width
            w_mask = (wi_offs >= 0) & (wi_offs < input_width)
            spatial_mask = h_valid & w_mask[:, None] & wo_mask

            # Clamp wi_offs to valid range for pointer safety (per-element).
            # Invalid reads are excluded by w_mask in spatial_mask.
            wi_safe = tl.minimum(tl.maximum(wi_offs, 0), input_width - 1)

            # Input: (BLOCK_WO, BLOCK_CI) - uses gather due to per-element wi_safe
            inp_ptrs = (
                input_base
                + hi_safe * input_height_stride
                + wi_safe[:, None] * input_width_stride
                + ci_offs[None, :] * input_c_stride
            )

            # Weight: regular 2D strided (BLOCK_CI, BLOCK_CO)
            wgt_ptrs = (
                weight_base
                + co_offs[None, :] * weight_n_stride
                + ci_offs[:, None] * weight_c_stride
                + h * weight_height_stride
                + w * weight_width_stride
            )

            for ci in range(BLOCK_CI_COUNT):
                c_mask = ci_offs + ci * BLOCK_CI < weight_c
                input_mask = spatial_mask & c_mask[None, :]
                weight_mask = c_mask[:, None] & co_mask
    ...
    # Bias: 1D load, broadcast
    bias_offs = pid_group * out_per_group_c + co_offs
    bias = tl.load(bias_pointer + bias_offs, mask=(co_offs < out_per_group_c)).to(
        tl.float32
    )
    accum += bias[None, :]

    # Output: regular 2D strided (BLOCK_WO, BLOCK_CO)
    out_ptrs = (
        output_pointer
        + n * output_n_stride
        + (pid_group * out_per_group_c + co_offs)[None, :] * output_c_stride
        + ho * output_height_stride
        + wo_offs[:, None] * output_width_stride
    )
    output_mask = wo_mask & co_mask
    tl.store(out_ptrs, accum, mask=output_mask)
...
class Conv2d(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias, stride, padding, dilation, groups):
        logging.debug("SOPHGO GEMS CONV2D")
        ...
        # BLOCK_NI_HO_WO acts as BLOCK_WO (only tiling out_width),
        # N and H_out are decomposed into grid dim 2 for regular strided access.
        grid = lambda META: (
            triton.cdiv(out_width, META["BLOCK_NI_HO_WO"]),
            triton.cdiv(int(out_c // groups), META["BLOCK_CO"]),
            groups * in_n * out_height,
        )
    ...
    def backward(ctx, out_grad):
        logging.debug("SOPHGO GEMS CONV2D VJP")
        ...
        conv2d_forward_kernel_gather[grid](
            new_out,
            revert_weight,
            input_back,
            bias_zero,
